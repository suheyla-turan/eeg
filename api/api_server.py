...
Endpoint'ler:
  ...
  POST /test/finalize
  GET  /test/status
  POST /analyze/session   # Gemini EEG yorumu
  WS   /ws/live
"""
...
async def analyze_session_endpoint(
    ...
    """
    Reels + Metin EEG metriklerini Gemini ile analiz eder.

    İstek gövdesi Flutter ExperimentResult / PhaseMetrics alanlarıyla uyumludur.
    Dakika serisi yoksa attentionSeries vb. epoch serilerinden türetilir.
    """
    result = await analyze_session_async(payload)
    if not result.ok and result.error and "GEMINI_API_KEY" in result.error:
