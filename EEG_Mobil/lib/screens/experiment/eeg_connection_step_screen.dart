...
class _EegConnectionStepScreenState extends State<EegConnectionStepScreen> {
  ...
  void dispose() {
    _finishTimer?.cancel();
    _eegWaitTimer?.cancel();
    _eeg?.removeListener(_onEegChanged);
    super.dispose();
  }

  void _armEegWaitTimer() {
    _eegWaitTimer?.cancel();
    _eegWaitTimedOut = false;
    // Cihaz bağlıysa EEG stream için makul süre bekle
    _eegWaitTimer = Timer(const Duration(seconds: 12), () {
      if (!mounted || _autoAdvanced || _finishing) return;
      final eeg = _eeg;
      if (eeg == null) return;
      if (eeg.canStartExperiment) return;
      if (eeg.connection == ConnectionStatus.connected) {
        setState(() => _eegWaitTimedOut = true);
      }
    });
  }
  ...
  void _onEegChanged() {
    ...
    if (eeg.canStartExperiment) {
      _eegWaitTimer?.cancel();
      _runFinishSequence();
    } else if (eeg.connection == ConnectionStatus.connected &&
        !_eegWaitTimedOut) {
      // Cihaz hazır, EEG bekleniyor — adım 3'te kal
      if (_visualStep < 3) {
        setState(() => _visualStep = 3);
      }
    }
  }

  int _mapConnectionToStep(EegProvider eeg) {
    if (eeg.canStartExperiment) return 4;
    switch (eeg.connection) {
      ...
      case ConnectionStatus.connected:
        // EEG stream gelene kadar "hazırlanıyor"
        return 3;
  ...
  void _runFinishSequence() {
    ...
    void tick() {
      ...
      if (step < 4) {
        ...
        _finishTimer = Timer(const Duration(milliseconds: 400), tick);
        return;
      }
      _autoAdvanced = true;
  ...
  String _statusMessage(EegProvider eeg) {
    if (eeg.isDemoMode) {
      return 'Demo modu açık — sahte EEG ile deneye devam edilecek.';
    }
    if (eeg.canStartExperiment) {
      return 'EEG stream aktif — deney başlıyor.';
    }
    if (eeg.connection == ConnectionStatus.connected) {
      if (_eegWaitTimedOut) {
        return 'Cihaz bağlı ama spektral veri gelmedi. '
            'API’yi yeniden başlatıp Yeniden Bağlan’a basın. '
            '(Ham EEG lisansı yoksa POW bant güçleri kullanılır.)';
      }
      return 'Cihaz bağlı — spektral veri (EEG/POW) bekleniyor…';
    }
    return 'EEG cihazı bağlandığında deney otomatik olarak devam eder.';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final deviceConnected =
        eeg.connection == ConnectionStatus.connected || eegReady;
          ...
          children: [
            ...
            Text(
              ...
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                    color: _eegWaitTimedOut && !eegReady
                        ? AppColors.danger
            ...
            Expanded(
              child: SingleChildScrollView(
                child: SectionCard(
                  ...
                  subtitle: eegReady
                      ? (eeg.isDemoMode ? 'Bağlı (Demo)' : 'EEG hazır')
                      : deviceConnected
                          ? 'Cihaz bağlı — EEG bekleniyor'
                          : eeg.connectionLabel,
                  icon: eegReady
                  ...
                  child: Column(
                    children: [
                      ...
                      if (deviceConnected) ...[
                        ...
                        Text(
                          'EEG stream: ${eeg.live.eegStreamActive ? "aktif" : "yok"} · '
                          'Spektral: ${eeg.live.hasSpectral ? "var" : "yok"}',
