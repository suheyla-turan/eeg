...
class SettingsService {
  ...
  static const _keyLastApiHost = 'api_last_successful_host';
  ...
  /// Son başarılı API host'u (otomatik hatırlama; override değil).
  String? get lastSuccessfulApiHost {
    final v = _prefs.getString(_keyLastApiHost);
    if (v == null || v.trim().isEmpty) return null;
    return v.trim();
  }

  Future<void> setLastSuccessfulApiHost(String host) async {
    final trimmed = host.trim();
    if (trimmed.isEmpty) {
      await _prefs.remove(_keyLastApiHost);
    } else {
      await _prefs.setString(_keyLastApiHost, trimmed);
    }
  }
