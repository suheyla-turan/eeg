...
class EegService {
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      Future<bool> tryHosts(Iterable<String> hosts) async {
      ...
      // Önce hızlı adaylar (mDNS yok) — USB / hatırlanan IP anında denensin.
      if (await tryHosts(_api.syncCandidateHosts())) return;

      final discovered = await _api.discoverMdnsHost();
      if (discovered != null &&
          await tryHosts([discovered])) {
        return;
      }
      ...
      _emit(
        LiveEegState.disconnected(
          error: _friendlyNetworkError(lastError),
        ),
      );
      AppLogger.instance.eeg(
  ...
  Future<void> _openSocket(String host) async {
    ...
    _channel = channel;
    _streaming = true;
    _api.rememberSuccessfulHost(host);
  ...
  void _onSocketClosed(String reason) {
    ...
    if (_disposed) return;
    if (_demoMode) return;
    _emit(LiveEegState.disconnected(error: _friendlyNetworkError(reason)));
    _scheduleReconnect();
  }

  /// Ham Socket/Timeout hatalarını kullanıcıya anlamlı Türkçe mesaja çevirir.
  String _friendlyNetworkError(Object? error) {
    ...
    if (lower.contains('timeout') ||
      ...
      return 'PC’deki Python API’ye ulaşılamadı (zaman aşımı). '
          'Telefon ile PC aynı Wi‑Fi’de olsun; Ayarlar’da host olarak '
          'PC IP’sini yaz (örn. 192.168.1.x) veya USB ile '
          '`adb reverse tcp:8000 tcp:8000` çalıştır.';
    }
    if (lower.contains('failed host lookup') ||
        lower.contains('no address associated') ||
        lower.contains('name not resolved')) {
      return 'API adresi çözülemedi (mDNS/eegserver.local). '
          'Ayarlar → Host override’a PC’nin Wi‑Fi IP’sini yaz.';
    }
    if (lower.contains('connection refused') ||
        lower.contains('connection reset')) {
      return 'PC’de API dinlemiyor veya yanlış adrese bağlanılıyor. '
          'API’nin çalıştığından emin ol; USB kullanıyorsan '
          '`adb reverse tcp:8000 tcp:8000` gerekli.';
    }
    if (raw.isNotEmpty) {
      return 'PC API bağlantısı kurulamadı: $raw';
    }
    return 'PC’deki Python API’ye bağlanılamadı (${EegApiConfig.displayUrl}).';
  }
