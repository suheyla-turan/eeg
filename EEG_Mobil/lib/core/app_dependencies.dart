...
import '../repositories/asset_video_repository.dart';
import '../repositories/firebase/firebase_video_watch_event_repository.dart';
import '../repositories/local_text_repository.dart';
...
import '../services/gemini_session_service.dart';
import '../services/result_reanalyzer.dart';

/// SOLID: bağımlılıklar soyutlamalara bağlanır (DI konteyneri).
///
/// Medya: [AssetVideoRepository] + [LocalTextRepository] (kod/assets).
/// Firebase: denek, deney, sonuç, watch/quiz yanıtları, EEG Storage.
class AppDependencies {
  AppDependencies({
    ...
    required this.resultReanalyzer,
    required this.geminiSessionService,
  });
  ...
  final ResultReanalyzer resultReanalyzer;
  final GeminiSessionService geminiSessionService;
  ...
  factory AppDependencies.create() {
    ...
    final videos = AssetVideoRepository();
    final texts = LocalTextRepository();
    ...
    final reanalyzer = ResultReanalyzer(
      storage: storage,
      results: results,
