...
class BrainMap extends StatelessWidget {
          ...
          child: Stack(
            ...
            children: [
              ...
              ...sensorIds.map((id) {
                ...
                // Dolgu: spektral bant (alpha/beta/…) — CQ bilişsel aktivite DEĞİL
                // Kenarlık: contact quality (elektrot teması)
                final power = _spectralActivity(bandPower);
                return Positioned(
  ...
  /// Global spektral bantlardan 0–1 aktivite (beta+gamma ağırlıklı).
  /// Contact quality kullanılmaz.
  double _spectralActivity(Map<String, double> bandPower) {
    final alpha = bandPower['alpha'] ?? 0;
    final beta = bandPower['beta'] ?? 0;
    final gamma = bandPower['gamma'] ?? 0;
    final theta = bandPower['theta'] ?? 0;
    final delta = bandPower['delta'] ?? 0;
    final total = alpha + beta + gamma + theta + delta;
    if (total <= 1e-12) return 0.15;
    // Relative beta+gamma → görsel uyarılma göstergesi
    return ((beta + gamma) / total).clamp(0.15, 1.0);
  }
