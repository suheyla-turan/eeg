...
class SettingsProvider extends ChangeNotifier {
  SettingsProvider({
    ...
    _applyApiConfig();
    EegApiConfig.onHostResolved = _persistSuccessfulHost;
    // Canlı EEG tick'lerinde (özellikle demo 10 Hz) MaterialApp /
    // Settings ağacını sürekli rebuild etme — yalnızca anlamlı değişimde.
    _liveSub = _eeg.liveStream.listen(_onLiveTick);
    _live = _eeg.latest;
    _loadPackageInfo();
  }
  ...
  void _applyApiConfig() {
    ...
    EegApiConfig.lastSuccessfulHost = _settings.lastSuccessfulApiHost;
  }

  Future<void> _persistSuccessfulHost(String host) async {
    await _settings.setLastSuccessfulApiHost(host);
    AppLogger.instance.python('API host hatırlandı: $host');
  }

  void _onLiveTick(LiveEegState state) {
    final prev = _live;
    _live = state;
    final connectionChanged = prev.connection != state.connection;
    final collectingChanged = prev.collecting != state.collecting;
    final batteryChanged =
        (prev.batteryPercent - state.batteryPercent).abs() >= 1;
    if (connectionChanged || collectingChanged || batteryChanged) {
      notifyListeners();
    }
  }
  ...
  @override
  void dispose() {
    if (EegApiConfig.onHostResolved == _persistSuccessfulHost) {
      EegApiConfig.onHostResolved = null;
    }
    _liveSub?.cancel();
    super.dispose();
  }
